...
export default function CreateInvoicePage() {
  ...
  function addItem() {
    setForm((f) => ({
      ...f,
      items: [...f.items, { article: '', qte: 1, prix_ht: 0, remise: 0, tva: 19, prix_ttc: 0 }],
    }));
  }
  ...
  function calculateTotals() {
    let total_ht = 0, total_tva = 0, total_remise_value = 0, total_ttc = 0;
    ...
    setTotals({ total_ht, total_tva, total_remise: total_remise_value, total_ttc });
  }
  ...
  function handleCreateNewInvoice() {
    setForm({
      client_id: '', project_id: '', invoice_number: '', ref_facture: '',
      issue_date: new Date().toISOString().slice(0, 10), due_date: '',
      timber: 1, payment_method: '', payment_status: 'impayé',
      items: [{ article: '', qte: 1, prix_ht: 0, remise: 0, tva: 19, prix_ttc: 0 }],
    });
      ...
      <div className={styles.container}>
        ...
        {isLoading ? <div>Chargement...</div> : (
          <form onSubmit={handleSubmit} className={styles.form}>
            ...
            <div className={styles.rowThree}>
              ...
              <div className={styles.formGroup}>
                <label>Timbre</label>
                <input type="number" value={Number(form.timber)} onChange={(e) => setForm({ ...form, timber: parseFloat(e.target.value) })} />
              </div>
            </div>
              ...
              <table className={styles.itemsTable}>
                ...
                <tbody>
                  {form.items.map((item, i) => (
                    <tr key={i}>
                      <td>{i + 1}</td>
                      <td><input type="text" value={item.article} onChange={(e) => handleItemChange(i, 'article', e.target.value)} /></td>
                      <td><input type="number" value={item.qte} onChange={(e) => handleItemChange(i, 'qte', e.target.value)} /></td>
                      <td><input type="number" value={item.prix_ht} onChange={(e) => handleItemChange(i, 'prix_ht', e.target.value)} /></td>
                      <td><input type="number" value={item.remise} onChange={(e) => handleItemChange(i, 'remise', e.target.value)} /></td>
                      <td><input type="number" value={item.tva} onChange={(e) => handleItemChange(i, 'tva', e.target.value)} /></td>
                      <td><input type="number" value={item.prix_ttc} onChange={(e) => handleItemChange(i, 'prix_ttc', e.target.value)} /></td>
                      <td>{(item.prix_ttc * item.qte).toFixed(3)}</td>
        ...
        {showDialog && (
          <div className={styles.dialog}>
            ...
            <p>✅ Facture enregistrée avec succès</p>
            <div>
